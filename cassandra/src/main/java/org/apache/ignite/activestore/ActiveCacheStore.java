...
public class ActiveCacheStore<K, V> implements CacheStore<K, V> {
    ...
    @Inject
    public ActiveCacheStore(MetadataProvider metadataProvider, KeyValueReader keyValueReader,
        List<KeyValueListener> allListeners, IdSequencer sequencer) {
    ...
    @Override public V load(K key) throws CacheLoaderException {
        return keyValueReader.load(key, cacheName, metadataProvider.path());
    }

    /** {@inheritDoc} */
    @Override public Map<K, V> loadAll(Iterable<? extends K> keys) throws CacheLoaderException {
        return keyValueReader.loadAll(keys, cacheName, metadataProvider.path());
    }
