...
/**
 * {@link Worker} implementation to test performance of single write operations.
 */
public class WriteWorker extends Worker {
    ...
    /** */
    public WriteWorker(Ignite ignite, LoadTestConfig config, long startPosition, long endPosition,
    ...
    @Override protected WorkerEntryProcessor getEntryProcessor(Ignite ignite, LoadTestConfig config) {
        return new WriteEntryProcessor(ignite, config);
    }

    /**
     * Processes a map of entries by putting them one by one into the cache.
     */
    public static class WriteEntryProcessor extends ServerSideEntryProcessor {
        public WriteEntryProcessor(Ignite ignite, LoadTestConfig config) {
            super(ignite, config);
        }
