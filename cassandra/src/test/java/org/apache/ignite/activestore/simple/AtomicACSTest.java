...
/**
 * Base test for atomic-based caches and active store.
 */
public abstract class AtomicACSTest extends ActiveCacheStoreTest {
    /** */
    @Test
    public void testWriteTwoAndNewSnapshot() {
        ...
        Assert.assertEquals("7", readKV("c", firstCache, snapshot1));
        Assert.assertEquals("8", readKV("d", firstCache, snapshot1));
        ...
        Assert.assertEquals(2, readKV(1, secondCache, snapshot1));
        Assert.assertEquals(3, readKV(2, secondCache, snapshot1));
    ...
    public void testRollbackTransaction() {
        ...
        Assert.assertEquals("57", result.get("c"));
        Assert.assertEquals("8", result.get("d"));

        Assert.assertEquals("5", readKV("a", firstCache, head));
        Assert.assertEquals("6", readKV("b", firstCache, head));
        Assert.assertEquals("57", readKV("c", firstCache, head));
        Assert.assertEquals("8", readKV("d", firstCache, head));
    }
}
