...
public class CassandraHelper {
    /** */
    private static final ResourceBundle CREDENTIALS = ResourceBundle.getBundle("org/apache/ignite/activestore/tests/cassandra/credentials");

    /** */
    private static final ResourceBundle CONNECTION = ResourceBundle.getBundle("org/apache/ignite/activestore/tests/cassandra/connection");

    /** */
    private static final ResourceBundle KEYSPACES = ResourceBundle.getBundle("org/apache/ignite/activestore/tests/cassandra/keyspaces");

    /** */
    private static final String EMBEDDED_CASSANDRA_YAML = "org/apache/ignite/activestore/tests/cassandra/embedded-cassandra.yaml";

    /** */
    private static final ApplicationContext connectionContext = new ClassPathXmlApplicationContext("org/apache/ignite/activestore/tests/cassandra/connection-settings.xml");
    ...
    /** */
    public static void clearTablesWithDefaultKeyspace(String... tables) {
        clearTables(getTestKeyspaces()[0], tables);
    }

    /** */
    public static void clearTableWithKeyspaceFromInjection(Ignite ignite, String... tables) {
        KeyValuePersistenceSettingsRegistry settings = Injection.get(KeyValuePersistenceSettingsRegistry.class, ignite);
        clearTables(settings.getKeyspace(), tables);
    }

    /** */
    private static void clearTables(String keyspace, String[] tables) {
        CassandraSession session = getAdminDataSrc().session(new NullLogger());
        for (String table : tables) {
            try {
                session.truncate(keyspace, table);
            }
            catch (Throwable e) {
                throw new RuntimeException("Failed to drop keyspace: " + keyspace, e);
            }
